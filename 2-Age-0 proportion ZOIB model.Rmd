---
title: "ZOIB Model for proportion of Age-0 Chinook salmon consumed by Steller sea lions along the Washington Coast"
...
```{r, echo = F}
library(readxl)
proportions <- read_xlsx("model inputs.xlsx", sheet = "Chinook age proportions")

knitr::kable(proportions, align = "l")
```


The following code details the loading of age-0 data, as determined in "1-CalculatedAge0.R". 
...
age_proportion <- read_xlsx("model inputs.xlsx", sheet = "Chinook age proportions")

metadata <- read_xlsx("model inputs.xlsx", sheet = "metadata")

RRA_age <-  merge(RRAdat, age_proportion, by = "season")

RRA_age$unkRRA <- (RRA_age$RRA*RRA_age$Other)             
RRA_age$Age0RRA<- (RRA_age$RRA*RRA_age$Age0)

RRA_age0 <- RRA_age %>% select(sample, Age0RRA)

model_df <- merge(RRA_age0, metadata, all = TRUE, by.x= "sample") %>% replace_na(list(Age0RRA=0))

model_df$diet_prop <- model_df$Age0RRA
model_df$species <- "age0"
```
...
# Build plot
prop_plot <- ggplot(model_df, aes(x=season, y=diet_prop)) + 
...
seasonal<- zoib(model_df$diet_prop ~ as.factor(season)|1|1, data=model_df, random = 0, EUID=model_df$sample, joint=FALSE, 
...
hist(winter_samples, xlim=c(0,0.04), ann=FALSE, yaxt="n", col = alpha(plot_col[1], 0.50), breaks=15)
mtext(side=2, font=2, line=2.0, "Winter", outer = FALSE)
hist(spring_samples, xlim=c(0,0.04), ann=FALSE, yaxt="n", col = alpha(plot_col[2], 0.50), breaks=15)
mtext(side=2, font=2, line=2.0, "Spring", outer = FALSE)
hist(summer_samples, xlim=c(0,0.04), ann=FALSE, yaxt="n", col = alpha(plot_col[3], 0.50), breaks=15)
...
write.csv(outs, file = "code outputs/2- modeled_age0_Chinook_samples.csv", row.names = FALSE)
```
